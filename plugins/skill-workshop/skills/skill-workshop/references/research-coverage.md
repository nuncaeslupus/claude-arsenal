...
|---|---|---|
| LLM-judge | The library does not run an LLM-judge to keep the validator fast and dependency-free. Semantic review happens conversationally inside this skill ("review this skill") and via scripted Claude Code probes in the smoke-test runner. | Live-session probes as a future extension of `plugins/skill-workshop/skills/skill-workshop/tests/skills_smoke.sh`, or a dedicated reviewer skill. |
...
- R-CADENCE-3 — Off-cadence triggers configured for 5 events (skill-workshop minor; validator major; ≥3 ROLLBACK; XPOLL-8 overlap; freshly-applied retro failure).
...
- R-DRIFT-1 — After N=3 accepted retros, invoke a skill-workshop description-optimisation pass.
...
- R-EXTRACT-2 — Extraction performed by skill-workshop.

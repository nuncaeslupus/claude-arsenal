...
| (plugin-layout) | must | Skill files live under `<scope>/.claude/skills/<name>/` (personal/project scope) or `plugins/<plugin>/skills/<name>/` (plugin scope). Plugin slugs in this marketplace are `skill-workshop` and `core`; new plugins land via `.claude-plugin/marketplace.json` + a per-plugin `.claude-plugin/plugin.json`. |

---
name: coverage-gaps
description: Use whenever the user wants to turn a coverage.py report into a ranked list of the highest-value missing tests — runs analyze_coverage.py over coverage.json and surfaces contiguous missing-line runs that mark whole untested functions or branches. Triggers — "what tests am I missing", "analyze my coverage", "where are the coverage gaps". Owns scripts — analyze_coverage. A cheap line-coverage pass below the mutmut-report skill. Do NOT use to run the suite or generate the report (run coverage / pytest --cov first), or for non-Python coverage.

...
0. **Refresh the bundle.**
   a. If `claude-arsenal/bin/check_update.sh` exists, run it **with `--check-only`** and
      ...
      **On a plugin install the report is INERT, and that is the correct answer, not a
      finding.** No remote and no subtree merge is what `/init` leaves behind; the script
      reports drift for subtree installs only. Say it once if it has not been said this
      session, and do not open work on it — updating means `/plugin update` then `/init`,

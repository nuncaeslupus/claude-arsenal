#!/usr/bin/env bash
# continue_smoke.sh — smoke test for pick_task.py (the /continue skill).
# Exit: 0 on PASS, 1 on FAIL.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PICK_PY="${SCRIPT_DIR}/../skills/continue/scripts/pick_task.py"
ADD_PY="${SCRIPT_DIR}/../skills/queue-add/scripts/create_task.py"
INIT_PY="${SCRIPT_DIR}/../skills/init/scripts/init.py"
BUNDLE_DIR="${SCRIPT_DIR}/../bundle"

for f in "${PICK_PY}" "${ADD_PY}" "${INIT_PY}"; do
    if [[ ! -f "${f}" ]]; then
...
# Gate 1: empty queue → pick_task exits 1
if python3 "${PICK_PY}" --queue "${QUEUE}" 2>/dev/null; then
    echo "FAIL: empty queue should exit 1" >&2; exit 1
fi

# Gate 2: add a task, pick_task returns it
ID1=$(python3 "${ADD_PY}" --title "Alpha task" --priority 10 --queue "${QUEUE}")
RESULT=$(python3 "${PICK_PY}" --queue "${QUEUE}")
if ! echo "${RESULT}" | python3 -c "import json,sys; d=json.load(sys.stdin); assert d['id'] == '${ID1}'" 2>/dev/null; then
    echo "FAIL: pick_task did not return task ${ID1}" >&2; exit 1
fi

# Gate 3: fuzzy search finds the task
MATCH=$(python3 "${PICK_PY}" --queue "${QUEUE}" --search "alpha")
...
if python3 "${PICK_PY}" --queue "${QUEUE}" --search "zzz_no_match" 2>/dev/null; then

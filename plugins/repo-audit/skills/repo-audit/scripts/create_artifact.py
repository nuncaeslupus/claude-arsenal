#!/usr/bin/env python3
"""Render a repo-audit artifact's HTML from structured findings.

Takes the deterministic part of building the audit output — the page shell,
theme tokens, Q&A accordion markup, tables, and the findings ledger — off
the model's plate, so a run doesn't re-derive several hundred lines of
CSS/HTML from scratch. The model still writes the actual analysis (the
`--input` JSON); this script only lays it out.
...
STATUS_LABEL = {"fixed": "fixed", "flagged": "flagged", "bug": "bug — your call"}
...
.pill.bug{background:var(--bad-soft); color:var(--bad);}

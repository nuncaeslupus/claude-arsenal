#!/usr/bin/env bash
# detect_surface.sh — writes .loop/state/surface_profile.json on every SessionStart.
...
#
# DUPLICATED ACROSS SKILLS (detect_surface.sh):
#   plugins/loop-orchestrator/hooks/detect_surface.sh  (runs as plugin SessionStart hook)
#   plugins/loop-orchestrator/core/scripts/detect_surface.sh  (installed into host repos by loop-init)

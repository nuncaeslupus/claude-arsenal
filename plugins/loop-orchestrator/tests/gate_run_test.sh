...
GATE_RUN="${SCRIPT_DIR}/../core/scripts/gate_run.sh"
...
mkdir -p "${tmpdir}/.loop/state/tasks"
...
cat > "${tmpdir}/.loop/state/tasks/lo-no-gate.md" <<'EOF'
# T1: No gate
...
cat > "${tmpdir}/.loop/state/tasks/lo-prose.md" <<'EOF'
# T2: Prose gate
...
cat > "${tmpdir}/.loop/state/tasks/lo-pass.md" <<'EOF'
# T3: Passing gate
...
cat > "${tmpdir}/.loop/state/tasks/lo-fail.md" <<'EOF'
# T4: Failing gate
...
cat > "${tmpdir}/.loop/state/tasks/lo-multi.md" <<'EOF'
# T5: Multi-line gate

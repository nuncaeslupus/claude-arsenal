#!/usr/bin/env bash
# queue_tools_test.sh — integration test for add_task.py + queue_status.py (T8 gate).
# Verifies that /loop-add appends a valid JSONL row and /loop-status reports correct counts.
...
if [[ -z "${ID1}" || -z "${ID2}" ]]; then
    echo "FAIL: add_task.py did not print task IDs" >&2; exit 1
fi
...
if python3 "${ADD_PY}" --title "Orphan" --deps "lo-0000" --queue "${QUEUE}" 2>/dev/null; then
    echo "FAIL: add_task should have rejected unknown dep lo-0000" >&2; exit 1
fi

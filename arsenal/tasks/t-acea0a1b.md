...
Gates: `offset_reparse_mismatches == 0`, `encoding_matrix_pass_rate == 1.0`, SC2 (<= 30 s / <= 400 MB on a 200 MB capture), SC3 (<= 1 s index-only), and a 0-token resident delta for repos without `extract`.
